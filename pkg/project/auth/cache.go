...
func NewAuthorizationCache(reviewer Reviewer,
	...
	result := &AuthorizationCache{
		...
		lastSyncResourceVersioner: namespaceInformer.Informer(),
		...
		policyLastSyncResourceVersioner: unionLastSyncResourceVersioner{clusterRoles.Informer(), clusterRoleBindings.Informer(), roles.Informer(), roleBindings.Informer()},
...
// synchronizeRoles synchronizes access over each role
func (ac *AuthorizationCache) synchronizeRoles(userSubjectRecordStore cache.Store, groupSubjectRecordStore cache.Store, reviewRecordStore cache.Store) {
	roles, err := ac.roleLister.List(labels.Everything())
	if err != nil {
		utilruntime.HandleError(err)
		return
	}
...
func (ac *AuthorizationCache) synchronizeRoleBindings(userSubjectRecordStore cache.Store, groupSubjectRecordStore cache.Store, reviewRecordStore cache.Store) {
	roleBindings, err := ac.roleBindingLister.List(labels.Everything())
	if err != nil {
		utilruntime.HandleError(err)
		return
	}
...
func (ac *AuthorizationCache) invalidateCache() bool {
	...
	if (len(ac.clusterRoleBindingResourceVersions) != len(temporaryVersions)) || !ac.clusterRoleBindingResourceVersions.HasAll(temporaryVersions.List()...) {
		invalidateCache = true
		ac.clusterRoleBindingResourceVersions = temporaryVersions
	}
	return invalidateCache
}
...
func (ac *AuthorizationCache) synchronize() {
	...
	ac.synchronizeRoles(userSubjectRecordStore, groupSubjectRecordStore, reviewRecordStore)
...
func (ac *AuthorizationCache) syncRequest(request *reviewRequest, userSubjectRecordStore cache.Store, groupSubjectRecordStore cache.Store, reviewRecordStore cache.Store) error {
	...
	subjects, err := ac.reviewer.Review(namespace)
	if err != nil {
		return err
	}
	...
	addSubjectsToNamespace(userSubjectRecordStore, users, namespace)
	addSubjectsToNamespace(groupSubjectRecordStore, groups, namespace)
	cacheReviewRecord(request, lastKnownValue, users, groups, reviewRecordStore)
	ac.notifyWatchers(namespace, lastKnownValue, sets.NewString(users...), sets.NewString(groups...))
	return nil
}
